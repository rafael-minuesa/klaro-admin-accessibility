...
**v1.1.2** (2026-09-23) - On WordPress.org SVN (r3710119). September 2026 review fixes (tracker https://github.com/rafael-minuesa/klaro-admin-accessibility/issues/18) plus five fixes from local testing, 16 PRs, each measured live. 1.1.0 and 1.1.1 are GitHub tags only. Gotchas: the local plugin folder is a SYMLINK to this repo (the site runs the checked-out branch); TinyMCE `content_style` is printed unescaped inside a double-quoted JS string, never use double quotes in that CSS; the `#wpbody-content` sweep does not reach `iframe_header()` pages or body-level dialogs, those have explicit palettes in admin.css.
