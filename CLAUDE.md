...
**v1.2.0** (2026-09-26) - On WordPress.org SVN (r3714050). High contrast rebuilt on one opaque zero-specificity base palette, large-text control sizing and never-split table headers (PR #33). Previous: **v1.1.2** (2026-09-23) - On WordPress.org SVN (r3710119). September 2026 review fixes (tracker https://github.com/rafael-minuesa/klaro-admin-accessibility/issues/18) plus five fixes from local testing, 16 PRs, each measured live. 1.1.0 and 1.1.1 are GitHub tags only. Gotchas: the local plugin folder is a SYMLINK to this repo (the site runs the checked-out branch); TinyMCE `content_style` is printed unescaped inside a double-quoted JS string, never use double quotes in that CSS; the high-contrast base palette covers only `#wpcontent`, so `iframe_header()` pages and body-level dialogs keep explicit palettes in admin.css.

**High contrast design (since 2026-09-26, PR #33):** one zero-specificity base rule, `:where(body.klaro-aa-high-contrast #wpcontent) :where(:not(img, video, canvas, iframe, picture, :empty, [style*="background"]))`, makes EVERY element opaque black with white text; every deliberate rule (links, buttons, tabs, focus, selected states) simply comes after it and wins. Do not go back to a list of tags or to `transparent`: a list leaves pockets that need one fix per screen, and transparent layers let dropdown panels and sticky bars show the page through. Keep `:empty` excluded: WordPress components draw empty, absolutely positioned layers ON TOP of content (`.components-elevation`, the Card/Surface shadow), and an opaque black one hid the whole WooCommerce Home task list. Check any change with the contrast + covering-layer audit over the 14 admin screens (Playwright script, recreated per session: text contrast against the first opaque background, plus positioned, opaque, textless layers overlapping text).
